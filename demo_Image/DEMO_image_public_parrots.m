...
fprintf('PC-SBL: time: %4.3f, square root of NMSE: %g\n',mean(t_PC),mean(ERR));
figure(5)
imshow(uint8(X22));
title('PC-SBL');
...
%% ================== BSBL-EM  ==============
    ...
    fprintf('BSBL-EM(learn correlation) : time: %4.3f, square root of NMSE: %g\n',mean(t_em),mean(mse_em));
    figure(6)
    imshow(uint8(X_BSBL));
    title('BSBL');
...
%     =========================================================
    ...
    fprintf('DivSBL: time: %4.3f, square root of NMSE: %g\n',mean(t_em_DV),mean(mse_em_DV));
...
fprintf('SBL: time: %4.3f, square root of NMSE: %g\n',mean(t_BCS),mean(mse_SBL));
...
fprintf('Group Lasso(CVX): time: %4.3f, square root of NMSE: %g\n',mean(t_group_lasso),mean(mse_group_lasso));
...
fprintf('Group BPDN: time: %4.3f, square root of NMSE: %g\n',mean(t_GBPDN),mean(mse_GBPDN));
...
fprintf('StructOMP: time: %4.3f, square root of NMSE: %g\n',mean(t_StrOMP),mean(mse_StrOMP));
